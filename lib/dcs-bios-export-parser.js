...
class DcsBiosExportParser extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    var { aircraftData, controls, addressLookup } =
      JsonParser.parseFiles(files);
  ...
  getControlValue(controlId, outputSuffix) {
    ...
    var output = control.outputs.find(
      (output) => output.suffix == outputSuffix
    );
          ...
          this._addressLookup[controlAddress].forEach((control) => {
            control.outputs.forEach((output) => {
              ...
              if (output.type == 'string') {
                controlValue = this._data.toString(
                  'utf8',
                  controlAddress,
                  controlAddress + output.max_length
                );
              }
              ...
              var valueHasChanged =
                this._options.emitAllUpdates || output.value != controlValue;
              var isAlreadyAdded = this._emitQueue.some(
                (item) => item.control == control && item.output == output
              );
